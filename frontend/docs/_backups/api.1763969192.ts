...
// Λίστα (δουλεύει και αν το backend δεν έχει server-side pagination)
export const listProperties = (params: { page?: number; pageSize?: number; includeDeleted?: boolean; q?: string } = {}) => {
...
// Δημιουργία: δοκιμάζει /properties, αλλιώς κάνει fallback σε /properties/create-simple
export const createProperty = async (payload: { title: string; address?: string; rent?: number }) => {
  try {
    return await request<Property>("/properties", { method: "POST", body: JSON.stringify(payload) });
  } catch (_e) {
    return await request<Property>("/properties/create-simple", { method: "POST", body: JSON.stringify(payload) });
  }
};
...
export default { storage, login, me, listProperties, createProperty, updateRent, delProperty, restoreProperty };
