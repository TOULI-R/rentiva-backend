...
type ListParams = {
  q?: string; includeDeleted?: boolean; page?: number; pageSize?: number;
...
function listProperties(p: ListParams = {}) {
  return request(`/properties${qs(p)}`);
}
...
function updateProperty(id: string, patch: Partial<{ title: string; address: string; rent: number }>) {
...
async function restoreProperty(id: string) {
  try {
    ...
    if (e?.status === 404) return request(`/properties/${id}/restore`, { method: "POST" });
    throw e;
  }
}

const api = { storage, request, login, me, listProperties, createProperty, updateProperty, delProperty, restoreProperty };
export default api;
